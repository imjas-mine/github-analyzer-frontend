function RepositoryCard({ repo, username, formatDate, onClick, isSelected }) {
  return (
    <div 
      onClick={onClick}
      className={`bg-gray-800/50 hover:bg-gray-800/70 border rounded-xl sm:rounded-2xl p-4 sm:p-6 transition-all cursor-pointer ${
        isSelected 
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-3 mb-2 sm:mb-3">
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 min-w-0">
          <h3 className="text-base sm:text-lg lg:text-xl font-bold text-white group-hover:text-blue-400 transition-colors truncate">
            {repo.name}
          </h3>
        ...
        <span className="text-gray-400 text-xs sm:text-sm whitespace-nowrap">{formatDate(repo.createdAt)}</span>
      </div>

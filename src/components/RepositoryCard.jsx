...
function RepositoryCard({ repo, username, formatDate, onClick, isSelected }) {
  ...
  return (
    <div
      onClick={onClick}
      className={`${isDark ? 'bg-gray-800/50 hover:bg-gray-800/70' : 'bg-white/70 hover:bg-white/90 shadow-sm hover:shadow-md'} border rounded-xl sm:rounded-2xl p-4 sm:p-6 transition-all cursor-pointer overflow-hidden ${isSelected
          ? `${isDark ? 'border-blue-500/50 bg-gray-800/70' : 'border-blue-400/60 bg-white/90'}`
    ...
    >
      ...
      <div className="flex flex-col gap-2 sm:gap-3 mb-2 sm:mb-3">
        ...
        <div className="flex items-start justify-between gap-3">
          <h3 className={`text-base sm:text-lg lg:text-xl font-bold ${isDark ? 'text-white' : 'text-gray-900'} group-hover:text-blue-400 transition-colors truncate flex-1 min-w-0`}>
            {repo.name}
          </h3>
          <span className={`${isDark ? 'text-gray-400' : 'text-gray-500'} text-xs sm:text-sm whitespace-nowrap flex-shrink-0`}>{formatDate(repo.createdAt)}</span>
        </div>
      ...
      <p className={`${isDark ? 'text-gray-400' : 'text-gray-500'} text-sm sm:text-base mb-3 sm:mb-4 line-clamp-2`}>{repo.description || 'No description provided.'}</p>
      ...
      <div className="flex items-center gap-4 sm:gap-6 text-xs sm:text-sm flex-wrap">
        {repo.primaryLanguage && (
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            ></div>
            <span className={`${isDark ? 'text-gray-300' : 'text-gray-600'}`}>{repo.primaryLanguage.name}</span>
          </div>
        )}
        <div className={`flex items-center gap-1.5 sm:gap-2 ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
